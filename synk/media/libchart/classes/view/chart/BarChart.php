<?php
	...
	class BarChart extends Chart {
		 ...
		 */
		 function isEmptyDataSet($minNumberOfPoint) {
			if (is_a($this->dataSet, "XYDataSet")) {
				$pointList = $this->dataSet->getPointList();
				$pointCount = count($pointList);
				return $pointCount < $minNumberOfPoint;
			} else if (is_a($this->dataSet, "XYSeriesDataSet")) {
		 ...
		 */
		 function getDataAsSerieList() {
			// Get the data as a series list
			$serieList = null;
			if (is_a($this->dataSet, "XYSeriesDataSet")) {
				$serieList = $this->dataSet->getSerieList();
			} else if (is_a($this->dataSet, "XYDataSet")) {
				$serieList = array();
