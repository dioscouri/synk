...
class plgSystemSynk_sections extends JPlugin 
{
	 ...
	 * @since	1.0
	 */
	function plgSystemSynk_sections(& $subject, $config) {
	...
	{
		$success = false;
		$sections_all 	= plgSystemSynk_sections::_getParameter( 'sections_all', '1' );		
		$sections_list = @preg_replace( '/\s/', '', plgSystemSynk_sections::_getParameter( 'sections_list', '' ) );
		$sections_array	= explode( ',', $sections_list );
		
		// check sections
		if ($sections_all == '1') { $success = true; }
		elseif (in_array($item->id, $sections_array)) { $success = true; }
	...
	*/
	...
	function &_executeSynchronization( $synk, &$options ) {
		...
		// load all the sections from the local server
		$localdb = JFactory::getDBO();
		$localdb->setQuery( "SELECT * FROM #__sections" );
		$sections = $localdb->loadObjectList();
		...
		for ($i=0; $i<count($sections); $i++) {
			...
			$synkdb->setQuery( "SELECT * FROM #__sections WHERE `id` = '".$item->id."' " );
			$target = $synkdb->loadObject();
			if (empty($target->id)) { $isnew = true; }
						
			if ($isnew) {
				// create new entry
				$synkdb->setQuery( "INSERT INTO #__sections SET `id` = '".$item->id."' " );
				if ( !$synkdb->query() ) { 
					$msg->message .= ' - '.JText::_( 'New Section Insertion Failed' ).': '.$synkdb->getErrorMsg();
					return false;
				}
			}
			...
			if ( !$table->store() ) { 
				$msg->message .= ' - '.JText::_( 'Update Section Failed' ).': '.$table->getError();
				return false;
			}
