 ...
 * @since       1.6
 */
class SynkQueryElement extends JObject
{
 ...
 * @since       1.6
 */
class SynkQuery extends JObject
{
    ...
    public function select($columns)
    {
        ...
        if (is_null($this->_select)) {
            $this->_select = new SynkQueryElement('SELECT', $columns);
        } else {
    ...
    public function from($tables)
    {
        if (is_null($this->_from)) {
            $this->_from = new SynkQueryElement('FROM', $tables);
        } else {
    ...
    public function join($type, $conditions)
    {
        ...
        $this->_join[] = new SynkQueryElement(strtoupper($type) . ' JOIN', $conditions);
    ...
    public function where($conditions, $glue='AND')
    {
        if (is_null($this->_where)) {
            ...
            $this->_where = new SynkQueryElement('WHERE', $conditions, "\n\t$glue ");
        } else {
    ...
    public function group($columns)
    {
        if (is_null($this->_group)) {
            $this->_group = new SynkQueryElement('GROUP BY', $columns);
        } else {
    ...
    public function having($conditions, $glue='AND')
    {
        if (is_null($this->_having)) {
            $glue = strtoupper($glue);
            $this->_having = new SynkQueryElement('HAVING', $conditions, "\n\t$glue ");
        } else {
    ...
    public function order($columns)
    {
        if (is_null($this->_order)) {
            $this->_order = new SynkQueryElement('ORDER BY', $columns);
        } else {
